...
import (
	...
	"qms/admin-service/internal/telemetry"
...
func main() {
	cfg := config.Load()
	shutdownTelemetry := telemetry.Setup("admin-service")
	defer func() {
	...
	otelHandler := otelhttp.NewHandler(httpapi.LoggingMiddleware(limiter.Middleware(handler.Routes())), "admin-service")
	server := &http.Server{
		Addr:         ":" + cfg.Port,
		Handler:      otelHandler,
